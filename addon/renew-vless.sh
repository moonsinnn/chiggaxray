...
EXPIRATION_DATE=$(grep -E "^### " "/usr/local/etc/xray/vlesswstls.json" | cut -d ' ' -f 3 | sed -n "${CLIENT_NUMBER}"p)
...
NEW_EXPIRATION_DAYS=$(( REMAINING_DAYS + EXPIRATION_DAYS ))
NEW_EXPIRATION_DATE=$(date -d "$NEW_EXPIRATION_DAYS days" +"%Y-%m-%d")

# Update the expiration date in the configuration files
sed -i "s/### $USER $EXPIRATION_DATE/### $USER $NEW_EXPIRATION_DATE/g" /usr/local/etc/xray/vlesswstls.json
sed -i "s/### $USER $EXPIRATION_DATE/### $USER $NEW_EXPIRATION_DATE/g" /usr/local/etc/xray/vlessgrpc.json
